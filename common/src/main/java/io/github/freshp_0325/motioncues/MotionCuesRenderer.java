...
final class MotionCuesRenderer {
   private static ShaderInstance adaptiveContrastShader;
   private static TextureTarget backgroundSnapshot;

   private MotionCuesRenderer() {
   }

   static void setAdaptiveContrastShader(Object shader) {
      adaptiveContrastShader = (ShaderInstance)shader;
   }
   ...
   static void renderDots(GuiGraphics graphics, MotionCuesConfig config, float alpha, double phaseX, double phaseY, float depthScale, float horizontalFlow) {
      ...
      boolean adaptive = adaptiveRequested && adaptiveContrastShader != null;
      boolean difference = config.colorMode.equals("DIFFERENCE") || adaptiveRequested && adaptiveContrastShader == null;
      if (adaptive) {
         graphics.flush();
      ...
      if (adaptive) {
         ShaderInstance shader = adaptiveContrastShader;
         RenderSystem.disableBlend();
         shader.setSampler("BackgroundSampler", backgroundSnapshot.getColorTextureId());
         shader.safeGetUniform("ScreenSize").set((float)backgroundSnapshot.viewWidth, (float)backgroundSnapshot.viewHeight);
         shader.safeGetUniform("LuminanceThreshold").set(config.adaptiveLuminanceThreshold);
         shader.safeGetUniform("TransitionWidth").set(config.adaptiveTransitionWidth);
   ...
   private static boolean captureBackground(Minecraft minecraft) {
      try {
         ...
         if (backgroundSnapshot == null || backgroundSnapshot.width != main.width || backgroundSnapshot.height != main.height) {
            ...
            backgroundSnapshot = new TextureTarget(main.width, main.height, false, Minecraft.ON_OSX);
